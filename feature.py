...
pygame.display.set_caption('HexaForge')
clock = pygame.time.Clock()
tank_img = pygame.image.load(resource_path("assets/hex_tank.png")).convert_alpha()
tank_img = pygame.transform.scale(tank_img, (80, 45))

player_tank = pygame.image.load(resource_path("assets/hex_tank.png")).convert_alpha()
player_tank = pygame.transform.scale(player_tank, (90, 50))

enemy_tank = pygame.transform.flip(player_tank, True, False)

menu_bg = pygame.image.load(resource_path("assets/menu.png")).convert()
menu_bg = pygame.transform.scale(menu_bg, (width, height))

game_bg = pygame.image.load(resource_path("assets/game.png")).convert()
...
def tank(x, y, turPos, tank):
    ...
    if tank == 1:
        gameWin.blit(player_tank, (x - 45, y - 40))
    else:
        gameWin.blit(enemy_tank, (x - 45, y - 40))
...
def controlsWin():
    ...
    while True:
        gameWin.fill('#ffffff')
        Green = (0, 255, 0)
        show_message("HexaForge", '#000000', -200, size="large")
...
def mainWindow ():
    button1 = Button("Play", (240, 500), font=22)
    button2 = Button("Main", (360, 500), font=22)
    button3 = Button("Quit", (470, 500), font=22)
    

    while True:
        gameWin.blit(menu_bg, (0, 0))
        Green = (0, 255, 0)
        show_message("HexaForge", '#000000', -200, size="large")
...
def show_health_bars(player_health, enemy_health):
    ...
    if player_health > 75:
        player_color = (180, 0, 0)
    elif player_health > 50:
        player_color = (220, 70, 0)
    else:
...
def mainGame():
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    ...
                    text = font.render("Power: " + str(fire_power) + "%", True, (255, 255, 255))
                    gameWin.blit(text, [width / 2, 0])
                    text = font.render("Height: " + str(currentTurPos), True, (255, 255, 255))
                    gameWin.blit(text, [width / 2, 20])
                    pygame.draw.rect(gameWin, (120, 0, 0), [xlocation, height - randomHeight, barrier_width, randomHeight])
                    gameWin.fill(
                                (70, 0, 0),
        ...
        gameWin.blit(text, [width / 2, 20])
        pygame.draw.rect(gameWin, (120, 0, 0), [xlocation, height - randomHeight, barrier_width, randomHeight])
        gameWin.fill((70, 0, 0),rect=[0,height - groundHeight, width, height])
        if player_health < 1 :
            game_over(0)
        elif enemy_health < 1:
            game_over(1)
